...
function useMetaMaskInternal(): UseMetaMaskState {
  ...
  useEffect(() => {
    return () => {
      ...
      const c = connectListenerRef.current;
      const d = disconnectListenerRef.current;
      const ch = chainChangedListenerRef.current;
      const a = accountsChangedListenerRef.current;
      if (current) {
        c && (current.off?.("connect", c), current.removeListener?.("connect", c));
        d && (current.off?.("disconnect", d), current.removeListener?.("disconnect", d));
        ch && (current.off?.("chainChanged", ch), current.removeListener?.("chainChanged", ch));
        a && (current.off?.("accountsChanged", a), current.removeListener?.("accountsChanged", a));
      }
      chainChangedListenerRef.current = undefined;
