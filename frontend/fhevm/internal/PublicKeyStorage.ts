...
function assertFhevmStoredPublicKey(
  ...
  if (!("publicKeyId" in value)) throw new Error(`FhevmStoredPublicKey.publicKeyId does not exist`);
  if (typeof (value as any).publicKeyId !== "string") throw new Error(`FhevmStoredPublicKey.publicKeyId must be a string`);
  if (!("publicKey" in value)) throw new Error(`FhevmStoredPublicKey.publicKey does not exist`);
  if (!((value as any).publicKey instanceof Uint8Array)) throw new Error(`FhevmStoredPublicKey.publicKey must be a Uint8Array`);
}
...
function assertFhevmStoredPublicParams(
  ...
  if (value === null) return;
  if (!("publicParamsId" in value)) throw new Error(`FhevmStoredPublicParams.publicParamsId does not exist`);
  if (typeof (value as any).publicParamsId !== "string") throw new Error(`FhevmStoredPublicParams.publicParamsId must be a string`);
  if (!("publicParams" in value)) throw new Error(`FhevmStoredPublicParams.publicParams does not exist`);
  if (!((value as any).publicParams instanceof Uint8Array)) throw new Error(`FhevmStoredPublicParams.publicParams must be a Uint8Array`);
}
...
export async function publicKeyStorageGet(aclAddress: `0x${string}`): Promise<{
  ...
  try {
    ...
    if ((pk as any)?.value) {
      assertFhevmStoredPublicKey((pk as any).value);
      storedPublicKey = (pk as any).value;
    }
  } catch {}
  ...
  try {
    ...
    if ((pp as any)?.value) {
      assertFhevmStoredPublicParams((pp as any).value);
      storedPublicParams = (pp as any).value;
    }
  } catch {}
