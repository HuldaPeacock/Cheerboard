...
export class FhevmDecryptionSignature {
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    if (!("publicKey" in s && typeof (s as any).publicKey === "string")) return false;
    if (!("privateKey" in s && typeof (s as any).privateKey === "string")) return false;
    if (!("signature" in s && typeof (s as any).signature === "string")) return false;
    if (!("startTimestamp" in s && typeof (s as any).startTimestamp === "number")) return false;
    if (!("durationDays" in s && typeof (s as any).durationDays === "number")) return false;
    if (!("contractAddresses" in s && Array.isArray((s as any).contractAddresses))) return false;
    for (let i = 0; i < (s as any).contractAddresses.length; ++i) {
      const v = (s as any).contractAddresses[i];
      if (typeof v !== "string" || !v.startsWith("0x")) return false;
    }
    if (!("userAddress" in s && typeof (s as any).userAddress === "string" && (s as any).userAddress.startsWith("0x"))) return false;
    if (!("eip712" in s && typeof (s as any).eip712 === "object" && (s as any).eip712 !== null)) return false;
    return true;
  }
